package com.example.fragmject.core.ui.components.calendar
...
data class CalendarDate(
    ...
    suspend fun addSchedule(text: String) {
        ...
        value.add(text)
        schedule.emit(value)
        ScheduleStore.setSchedule(year, month, day, value)
    }
    ...
    private fun lunar(): LunarDate {
        return lunarDate ?: getLunarDate(year, month, day).also {
            AppScope.launch {
                //保证日程只从数据库取一次
                schedule.emit(ScheduleStore.getSchedule(year, month, day))
            }
            lunarDate = it
        }
    }

...
fun NavScreen(
    ...
    Column {
        ...
        HorizontalPager(state = pagerState) { page ->
            if (page == 0) {
                NavLinkContent(actions = actions)
            } else if (page == 1) {
                NavSystemContent(
...
fun NavLinkContent(
    ...
    SkeletonContent(uiState.isLoading) {
        Row {
            LazyColumn(
            ...
            VerticalPager(state = pagerState, userScrollEnabled = false) { page ->
                ...
                // flingBehavior 依赖 gridState/pagerState，用 remember 缓存：
                // 否则每次重组都会新建 FlingBehavior 实例（含 ScrollableDefaults.flingBehavior()），
                // 既浪费分配，又会导致嵌套滚动（fling 到相邻页）的状态丢失。
                val defaultFlingBehavior = ScrollableDefaults.flingBehavior()
                val flingBehavior = remember(pagerState, gridState) {
                ...
                LazyVerticalGrid(
                    ...
                    items(
                        ...
                        Button(
                            onClick = { actions.onArticleClick(item.link) },
                    ...
                    FlowRow(
                        ...
                        data.children?.forEach { children ->
                            Box(modifier = Modifier.padding(15.dp, 0.dp, 15.dp, 0.dp)) {
                                Button(
                                    onClick = { actions.onChapterClick(children.id) },
                                    shape = RoundedCornerShape(50),
                                    colors = ButtonDefaults.buttonColors(
                                        containerColor = MaterialTheme.colorScheme.surfaceContainerHigh,
                                        contentColor = MaterialTheme.colorScheme.onPrimaryContainer
